...
def get_installed_distributions(
    ...
    return [
        d for d in working_set if local_test(d) and d.key not in skip
        and editable_test(d) and editables_only_test(d) and user_test(d)
    ]
...
def split_auth_from_netloc(netloc):
    ...
    user_pass = tuple(None if x is None else urllib.parse.unquote(x)
                      for x in user_pass)
