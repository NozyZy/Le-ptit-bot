    ...
    for i, item in enumerate(sl):
        ...
        if keyname:
            ...
            if openstring:
                ...
                if item.isspace():
                    ...
                    continue
                elif dottedkey and sl[i - 1] == "." and (item == '"'
                                                         or item == "'"):
                    openstring = True
                    openstrchar = item
                    continue
        ...
        if item == "\n":
            if openstring or multilinestr:
                ...
                if (sl[i - 1] == "'" or sl[i - 1] == '"') and (sl[i - 2]
                                                               == sl[i - 1]):
                    sl[i] = sl[i - 1]
                    if sl[i - 3] == sl[i - 1]:
                        sl[i - 3] = " "
        ...
        if line[0] == "[":
            ...
            while i < len(groups):
                ...
                if len(groups[i]) > 0 and (groups[i][0] == '"'
                    ...
                    groupstr = groups[i]
                    j = i + 1
                    while (not groupstr[0]
                           == groupstr[-1]) or len(groupstr) == 1:
                        j += 1
                        if j > len(groups) + 2:
                            raise TomlDecodeError(
...
class TomlDecoder(object):
    ...
    def load_inline_object(self,
        ...
        while len(candidate_groups) > 0:
            ...
            if (value[0] == value[-1] and value[0] in ('"', "'")) or (
                ...
                candidate_groups[
                    0] = candidate_group + "," + candidate_groups[0]
            else:
    ...
    def load_line(self, line, currentlevel, multikey, multibackslash):
        ...
        elif (pair[0][0] == '"' or pair[0][0] == "'") and (pair[0][-1]
                                                           == pair[0][0]):
