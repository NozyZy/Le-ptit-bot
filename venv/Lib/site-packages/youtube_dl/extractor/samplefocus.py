...
class SampleFocusIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        title = self._og_search_title(webpage,
                                      fatal=False) or self._html_search_regex(
        ...
        mobj = re.search(r'>By <a[^>]+href="/users/([^"]+)"[^>]*>([^<]+)',
                         webpage)
        if mobj:
            uploader_id, uploader = mobj.groups()

        breadcrumb = get_element_by_attribute("typeof", "BreadcrumbList",
                                              webpage)
        categories = []
        if breadcrumb:
            for _, name in re.findall(
                    r'<span[^>]+property=(["\'])name\1[^>]*>([^<]+)',
                    breadcrumb):
        ...
        return {
            ...
            uploader_id,
            "like_count":
            extract_count("sample-%s-favorites" % sample_id),
            "comment_count":
