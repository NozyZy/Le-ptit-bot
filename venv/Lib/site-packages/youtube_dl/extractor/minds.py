...
class MindsIE(MindsBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entity = self._call_api("v1/entities/entity/" + entity_id, entity_id,
                                "entity")["entity"]
        ...
        return {
            ...
            video_id,
            "title":
            entity.get("title") or video_id,
            "formats":
            formats,
            "description":
            clean_html(entity.get("description")) or None,
            "license":
            str_or_none(entity.get("license")),
            "timestamp":
            int_or_none(entity.get("time_created")),
            "uploader":
            strip_or_none(owner.get("name")),
            "uploader_id":
            uploader_id,
            "uploader_url":
            "https://www.minds.com/" + uploader_id if uploader_id else None,
            "view_count":
            int_or_none(entity.get("play:count")),
            "like_count":
            int_or_none(entity.get("thumbs:up:count")),
            "dislike_count":
            int_or_none(entity.get("thumbs:down:count")),
            "tags":
            tags,
            "comment_count":
            int_or_none(entity.get("comments:count")),
            "thumbnail":
            thumbnail,
        }
...
class MindsFeedBaseIE(MindsBaseIE):
    ...
    def _entries(self, feed_id):
        ...
        i = 1
        while True:
            data = self._call_api("v2/feeds/container/%s/videos" % feed_id,
            ...
            for entity in entities:
                ...
                if not guid:
                    continue
                yield self.url_result("https://www.minds.com/newsfeed/" + guid,
                                      MindsIE.ie_key(), guid)
            query["from_timestamp"] = data["load-next"]
            if not (query["from_timestamp"]
                    and len(entities) == self._PAGE_SIZE):
                break
            i += 1

    def _real_extract(self, url):
        feed_id = self._match_id(url)
        feed = self._call_api("v1/%s/%s" % (self._FEED_PATH, feed_id), feed_id,
                              self._FEED_TYPE)[self._FEED_TYPE]
...
class MindsGroupIE(MindsFeedBaseIE):
    ...
    _TEST = {
        "url":
        "https://www.minds.com/groups/profile/785582576369672204/feed/videos",
        "info_dict": {
