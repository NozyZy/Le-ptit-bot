...
class TrovoVodIE(TrovoBaseIE):
    ...
    _TESTS = [
        {
            "url":
            "https://trovo.live/video/ltv-100095501_100095501_1609596043",
            "info_dict": {
      ...
      parentID
        ...
        for play_info in vod_info.get("playInfos") or []:
            ...
            formats.append({
                ...
                "mp4",
                "filesize":
                int_or_none(play_info.get("fileSize")),
                "format_id":
                format_id,
                "height":
                int_or_none(format_id[:-1]) if format_id else None,
                ...
                "m3u8_native",
                "tbr":
                int_or_none(play_info.get("bitrate")),
                "url":
                play_url,
                "http_headers": {
        ...
        def get_count(x):
            return int_or_none(vod_info.get(x + "Num"))

        comment_list = (try_get(
            resp, lambda x: x[1]["data"]["getCommentList"]["commentList"],
            list) or [])
        comments = []
        for comment in comment_list:
            ...
            comments.append({
                ...
                author.get("nickName"),
                "author_id":
                str_or_none(author.get("uid")),
                "id":
                str_or_none(comment.get("commentID")),
                "text":
                content,
                "timestamp":
                int_or_none(comment.get("createdAt")),
                "parent":
                "root" if parent == 0 else str_or_none(parent),
            })
